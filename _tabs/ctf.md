---
layout: page
title: CTF
icon: fas fa-flag        # font awesome icon shown in sidebar
order: 4                # controls position in sidebar nav
---

An overview of CTFs I've participated in and challenges solved.

## Advent of CTF 2025
**Date:** Dec 2025

| Challenge | Category | Difficulty |
|-----------|----------|------------|
| Syndicate Infrastructure | Misc | NA |
| Kramazon | Web | NA |

## Batman's Kitchen CTF 2026
**Date**: 21 Feb 2026 - 23 Feb 2026

| Challenge | Category | Difficulty |
|-----------|----------|------------|
| [Cortex Decay](/posts/bkctf-cortex-decay/) | Crypto | Easy |
| Y2K Time Capsule | Crypto | Medium |
| [Egghead](/posts/bkctf-egghead/) | Pwn | Easy |
| [My First Blog](/posts/my-first-blog/) | Web | Easy |
| [osint](/posts/more-like-ohshitamirite) | Osint | Easy |
| MOM GET THE CAMERA | Osint | Medium |